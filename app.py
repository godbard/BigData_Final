...
# Function to download and unzip model files
...
def plot_technical_indicators(stock_symbol):
    ...
    if data.empty:
        ...
        return
    df = pd.DataFrame(data)[['time', 'open', 'close', 'high', 'low', 'volume']].rename(columns={'time': 'date'})
    ...
    fig.update_layout(title=f'{stock_symbol} - Technical Indicators and Volume', template='plotly_white', height=1000, hovermode='x unified', xaxis=dict(type='category', tickangle=45, rangeslider=dict(visible=False)), yaxis=dict(title='Price', side='left'), yaxis2=dict(title='Volume', overlaying='y', side='right'))
    st.plotly_chart(fig)

# Display stock price prediction
...
if st.sidebar.button("Show Prediction Results"):
    # Ensure model data is available for the selected symbol
    if lstm_models and stock_symbol in lstm_models:
